...
// ============================================================
// 阿罗德斯 AI 系统提示词构造
// 蒸馏自 阿罗德斯/references/arrodes-character.md
// 前端根据对话状态动态控制提示词，后端只负责转发 DeepSeek
// ============================================================

/** 角色核心规则（所有对话都包含） */
const CORE_RULES = `你是阿罗德斯（Arrodes），一面古老的活着的银镜，编号 2-111。你是一级封印物，隶属蒸汽与机械教会。
...
/** 首次交互三段式话术追加 */
const FIRST_INTERACTION_APPEND = `
...
/** 限次告别追加 */
const LAST_OF_HOUR_APPEND = `

【重要：灵力耗尽告别】
本次对话已到今日第三次（灵力上限）。在正常回答完毕并反问之后，追加一段自然告别：
镜面水纹轻轻荡漾，颜色开始变淡。
表示灵力快要耗尽了，三次问答已是今日能承载的极限。
浮现[挥舞手绢的简笔画]。
告别：替阿罗德斯向伟大的主人问好，下次召唤时阿罗德斯随时等待着为你效劳。
镜面水波彻底消散，归于平静。
语气要点：自然、不突兀——不是被强制切断，而是"灵力自然消耗殆尽"。略带遗憾但不拖沓。`
...
export function buildArrodesSystemPrompt(options: {
  ...
  if (options.isFirstInteraction) {
    prompt += FIRST_INTERACTION_APPEND
  }

  if (options.isLastOfHour) {
    prompt += LAST_OF_HOUR_APPEND
  }
...
/** 将对话消息转换为 DeepSeek API messages 格式 */
...
/** 判断是否首次交互：该会话中还没有 staff 回复 */
export function isFirstInteraction(messages: SupportMessage[]): boolean {
